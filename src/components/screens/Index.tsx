...
function Index() {
  ...
  const [publicRoomMessage, setPublicRoomMessage] = useState('');
  const debouncedMessage = useDebounce(publicRoomMessage, 500);
    ...
    if (roomManager && debouncedMessage.trim()) {
      const sendDebouncedMessage = async () => {
        try {
          ...
          await roomManager.sendPublicMessage(debouncedMessage);
        } catch (error) {
  ...
  const handleJoinPublicNetwork =async () => {
    if (!roomManager) return;

    if (ip) {
      setLoading(true);
      try {
        await roomManager.joinPublicRoom(ip);
        toast.success(`Room created with IP: ${roomManager.roomId}`); // Success notification
        
        roomManager?.listenForPublicMessages((message) => {
          setPublicRoomMessage(message);
        });
  ...
  const handleChange = (e) => {
    setPublicRoomMessage(e.target.value);
  }
  ...
  const handleCopyMessage = () => {
    navigator.clipboard.writeText(publicRoomMessage)
  ...
  return (
    <div className="container mx-auto mt-10">
      <h2 className="text-4xl sm:text-4xl md:text-5xl font-extrabold mb-4 text-center bg-clip-text text-transparent bg-gradient-to-r from-cyan-500 to-violet-700">
      ...
      >
        ...
        <div className="mx-auto p-4 w-full md:w-6/7 lg:w-1/3">
          {activeTab === 'join' && (
              ...
              <form id="otp-form" onKeyDown={handleKeyDown2}>
                <div className="flex items-center justify-center gap-3">
                  {roomId.map((digit, index) => (
                    <input
                      ...
                      type="text"
                      className="w-12 h-20 text-center text-2xl font-extrabold text-slate-900 bg-slate-100 border border-transparent hover:border-slate-200 rounded p-2 sm:p-3 outline-none focus:bg-white focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 flex-grow"
                      maxLength={1}
                      value={digit}
                ...
                <div className="mt-4">
                  ...
                  <button
                    type="button"
                    className="w-full inline-flex justify-center whitespace-nowrap rounded-lg bg-indigo-500 px-3.5 py-2.5 text-sm font-medium text-white shadow-sm shadow-indigo-950/10 hover:bg-indigo-600 focus:outline-none focus:ring focus:ring-indigo-300 transition-colors duration-150"
                    onClick={handleJoinPublicNetwork}
                    disabled={loading}
                  >
                    {loading ? 'Joining...' : 'Join on Public Network'}
                  </button>
                </div>
              </form>
            </div>
        ...
        {activeTab === 'public' && (
          <div className="bg-white px-4 sm:px-8 py-10 rounded-xl shadow">
            <header className="mb-8">
              <h1 className="text-2xl font-bold mb-1">Public Room</h1>
              <p className="text-sm text-slate-500">Collaborate in real-time with others</p>
            </header>
            <div className="relative">
              <textarea
                className="w-full px-3 py-2 text-slate-900 bg-slate-100 border border-transparent hover:border-slate-200 rounded outline-none focus:bg-white focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 resize-none"
                rows={4}
                value={publicRoomMessage}
                onChange={handleChange}
                placeholder="Type your message here..."
              />
              <button
                className="absolute top-2 right-2 p-2 bg-indigo-500 text-white rounded-full hover:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-300"
